...
def is_torrent_active():

    try:
        result = subprocess.run(['pgrep', '-x', 'transmission-da'], capture_output=True, text=True)
        return result.returncode == 0

    except Exception as e:
        log.error("Torrent check failed: {str(e)}")
        return False
...
def switch_vpn():
    ...
    while True:
        ...
        if success:
            consecutive_failures = 0
            if is_torrent_active():
                log.info("Torrent detected - using extended switch window")
                interval =  random.randint(10800, 18000) #3-5 hours
            else:
                interval = random.randint(900, 2700) #normal 15-45 min 

            log.info(f"VPN active. Next switch in {interval//60} minutes")

            for _ in range (interval // 60):
                if not is_torrent_active(): #early exit if torrent finishes
                    break
                time.sleep(60) #will check every min

        else:
            consecutive_failures += 1
            retry_delay = min(300 * consecutive_failures, 3600)
            log.warning(f"Will retry in {retry_delay//60} min :(failures: {consecutive_failures})")
